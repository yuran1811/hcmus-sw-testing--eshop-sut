...
- Name: coupon-admin.spec.js >> Coupon Admin (FR-17) E2E Tests >> TC-COUPON-ADMIN-017: Đăng nhập với role = user (non-admin) → 403 Forbidden
- Location: coupon-admin.spec.js:470:3
...
> 489 |     expect(res.status()).toBe(403);
  ...
  513 |   });
  514 |
  515 |   // ── TC-019: discount_value = 2 (BVA IN) → chấp nhận ───────────────────────
  516 |
  517 |   test('TC-COUPON-ADMIN-019: discount_value = 2 (IN, vừa trên min) → được chấp nhận', async ({ page }) => {
  518 |     await loginAdmin(page);
  519 |     await navigateToCoupons(page);
  520 |
  521 |     await fillCouponForm(page, {
  522 |       code: 'DVIN',
  523 |       type: 'percent',
  524 |       discount_value: 2,
  525 |       min_order_amount: 0,
  526 |       expired_at: '2099-12-31',
  527 |       max_uses_per_user: 1,
  528 |     });
  529 |
  530 |     await page.click('button:has-text("Tạo mã")');
  531 |     const row = page.locator('table tbody tr td.font-mono').filter({ hasText: 'DVIN' });
  532 |     await expect(row).toBeVisible({ timeout: 5000 });
  533 |     await runQuery("DELETE FROM coupons WHERE code = 'DVIN'");
  534 |   });
  535 |
  536 |   // ── TC-020: min_order_amount = 0 (BVA ON) → chấp nhận ──────────────────────
  537 |
  538 |   test('TC-COUPON-ADMIN-020: min_order_amount = 0 (ON, đúng min ≥ 0) → được chấp nhận', async ({ page }) => {
  539 |     await loginAdmin(page);
  540 |     await navigateToCoupons(page);
  541 |
  542 |     await fillCouponForm(page, {
  543 |       code: 'MOAON',
  544 |       type: 'percent',
  545 |       discount_value: 10,
  546 |       min_order_amount: 0,
  547 |       expired_at: '2099-12-31',
  548 |       max_uses_per_user: 1,
  549 |     });
  550 |
  551 |     await page.click('button:has-text("Tạo mã")');
  552 |     const row = page.locator('table tbody tr td.font-mono').filter({ hasText: 'MOAON' });
  553 |     await expect(row).toBeVisible({ timeout: 5000 });
  554 |     await runQuery("DELETE FROM coupons WHERE code = 'MOAON'");
  555 |   });
  556 |
  557 |   // ── TC-021: min_order_amount = 1 (BVA IN) → chấp nhận ──────────────────────
  558 |
  559 |   test('TC-COUPON-ADMIN-021: min_order_amount = 1 (IN, vừa trên min) → được chấp nhận', async ({ page }) => {
  560 |     await loginAdmin(page);
  561 |     await navigateToCoupons(page);
  562 |
  563 |     await fillCouponForm(page, {
  564 |       code: 'MOAIN',
  565 |       type: 'percent',
  566 |       discount_value: 10,
  567 |       min_order_amount: 1,
  568 |       expired_at: '2099-12-31',
  569 |       max_uses_per_user: 1,
  570 |     });
  571 |
  572 |     await page.click('button:has-text("Tạo mã")');
  573 |     const row = page.locator('table tbody tr td.font-mono').filter({ hasText: 'MOAIN' });
  574 |     await expect(row).toBeVisible({ timeout: 5000 });
  575 |     await runQuery("DELETE FROM coupons WHERE code = 'MOAIN'");
  576 |   });
  577 |
  578 |   // ── TC-022: max_uses_per_user = 1 (BVA ON) → chấp nhận ─────────────────────
  579 |
  580 |   test('TC-COUPON-ADMIN-022: max_uses_per_user = 1 (ON, đúng min ≥ 1) → được chấp nhận', async ({ page }) => {
  581 |     await loginAdmin(page);
  582 |     await navigateToCoupons(page);
  583 |
