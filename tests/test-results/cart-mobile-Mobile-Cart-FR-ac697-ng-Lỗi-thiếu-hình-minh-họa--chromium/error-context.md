...
- Name: cart-mobile.spec.js >> Mobile Cart (FR-20) E2E Tests >> TC-CART-MOBILE-024: Kiểm tra giao diện khi giỏ hàng trống (Lỗi thiếu hình minh họa)
- Location: cart-mobile.spec.js:461:3
...
Locator: locator('div').filter({ has: locator('text="Giỏ hàng của bạn đang trống"') }).last().locator('img, svg')
Expected: visible
Timeout: 1000ms
...
Call log:
  ...
  - waiting for locator('div').filter({ has: locator('text="Giỏ hàng của bạn đang trống"') }).last().locator('img, svg')

```

```yaml
- text: EShop Mobile Chào, Test User Giỏ (0) Giỏ Hàng Giỏ hàng của bạn đang trống Tiếp tục mua sắm © 2026 EShop SUT. Dành cho mục đích kiểm thử.
```
