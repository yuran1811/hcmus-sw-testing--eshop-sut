...
- Name: cart-mobile.spec.js >> Mobile Cart (FR-20) E2E Tests >> TC-CART-MOBILE-022: Kiểm tra định dạng hiển thị tiền tệ, màu sắc nút bấm và nhãn hiển thị Tổng cộng (Lỗi nhãn)
- Location: cart-mobile.spec.js:417:3
...
Locator: locator('text="Giá: 100.000 ₫"')
Expected: visible
Timeout: 3000ms
...
Call log:
  ...
  - waiting for locator('text="Giá: 100.000 ₫"')

```

```yaml
- text: 'EShop Mobile Chào, Test User Giỏ (1) Giỏ Hàng Sản phẩm A Giá: 100,000 ₫ Số lượng:'
- textbox: '1'
- text: 'Thành tiền: 100,000 ₫ Xóa Tổng tạm tính: 100,000 ₫ ← Mua tiếp Tiến hành thanh toán © 2026 EShop SUT. Dành cho mục đích kiểm thử.'
```
...
```ts
  328 |
  329 |     await targetProduct(page, 'Sản phẩm A');
  330 |     await page.locator('text="Thêm vào giỏ"').click();
  331 |
  332 |     await page.locator('text=/^Giỏ \\(1\\)/').click();
  333 |
  334 |     // Click "Tiến hành thanh toán"
  335 |     const checkoutBtn = page.locator('text="Tiến hành thanh toán"');
  336 |     await checkoutBtn.click();
  337 |
  338 |     // Should redirect to Login screen
  339 |     await expect(page.locator('text="Đăng Nhập"')).toBeVisible();
  340 |   });
  341 |
  342 |   test('TC-CART-MOBILE-017: Đã đăng nhập bấm "Tiến hành thanh toán" -> chuyển hướng đến Checkout page thành công', async ({ page }) => {
  343 |     await loginMobileAdmin(page);
  344 |     await targetProduct(page, 'Sản phẩm A');
  345 |
  346 |     await page.locator('text="Thêm vào giỏ"').click();
  347 |
  348 |     await page.locator('text=/^Giỏ \\(1\\)/').click();
  349 |     await page.click('text="Tiến hành thanh toán"');
  350 |
