# TC-CHECKOUT-BVA-002: Thanh toán đơn hàng thất bại khi tổng tiền client gửi ít hơn máy chủ tính đúng 1 đơn vị
...
| Request Body         | `{"total_amount": 9999999, "shipping_address": "123 Le Loi, TP.HCM"}` |

## Test steps

1. Đăng nhập và lấy Token JWT hợp lệ.
2. Thêm các sản phẩm trên vào giỏ hàng (tổng tiền 10.000.000 ₫).
3. Gửi yêu cầu POST tới `/api/checkout` với Token JWT và Request Body chứa `total_amount = 9999999` (ít hơn 1 đơn vị) và `shipping_address = "123 Le Loi, TP.HCM"`.
...
- Không có đơn hàng nào được tạo trong cơ sở dữ liệu (hoặc tổng tiền được lưu là 10.000.000 ₫, không được lưu 9.999.999 ₫).
