...
| Request Body         | `{"total_amount": 1000, "shipping_address": "123 Le Loi, TP.HCM"}` |

## Test steps

1. Đăng nhập và lấy Token JWT hợp lệ.
2. Thêm sản phẩm vào giỏ hàng sao cho tổng tiền thực tế là 10.000.000 ₫.
3. Gửi yêu cầu POST tới `/api/checkout` với Token JWT trong header và Request Body chứa `total_amount = 1000` (giá trị không khớp nhằm gian lận) và `shipping_address = "123 Le Loi, TP.HCM"`.
