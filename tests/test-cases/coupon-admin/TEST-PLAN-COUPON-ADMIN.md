...
| Invalid | EP-DV-2   | `0`                  | Bằng 0 (không dương) |
...
| Domain  | Partition | Representative Value | Mô tả           |
| ------- | --------- | -------------------- | --------------- |
| Valid   | EP-MOA-1  | `0`                  | Bằng 0 (hợp lệ) |
| Valid   | EP-MOA-2  | `200000`             | Số dương        |
| Invalid | EP-MOA-3  | `-1`                 | Số âm           |

### V6 — max_uses_per_user (số lần dùng tối đa mỗi người)

| Domain  | Partition | Representative Value | Mô tả                         |
| ------- | --------- | -------------------- | ----------------------------- |
| Valid   | EP-MPU-1  | `1`                  | Đúng ngưỡng tối thiểu (min=1) |
| Valid   | EP-MPU-2  | `5`                  | Số nguyên dương > 1           |
| Invalid | EP-MPU-3  | `0`                  | Bằng 0 (không đủ điều kiện)   |
...
| TC-COUPON-ADMIN-006 | Domain Testing | discount_value = 0 → bị từ chối                                          | Not Run |
...
| TC-COUPON-ADMIN-009 | Domain Testing | expired_at = định dạng ngày không hợp lệ → bị từ chối                    | Not Run |
| TC-COUPON-ADMIN-010 | Domain Testing | min_order_amount = số âm → bị từ chối                                    | Not Run |
| TC-COUPON-ADMIN-011 | Domain Testing | max_uses_per_user = 0 → bị từ chối                                       | Not Run |
...
| TC-COUPON-ADMIN-014 | Domain Testing | Xóa coupon theo ID không tồn tại → trả về lỗi 404                        | Not Run |
| TC-COUPON-ADMIN-015 | Domain Testing | Admin lấy danh sách coupon → trả về đầy đủ các trường (kể cả các mã mẫu) | Not Run |
...
| TC-COUPON-ADMIN-018 | BVA            | discount_value = 0 (OFF, phải > 0) → bị từ chối                          | Not Run |
| TC-COUPON-ADMIN-019 | BVA            | discount_value = 1 (ON, đúng min > 0) → được chấp nhận                   | Not Run |
| TC-COUPON-ADMIN-020 | BVA            | discount_value = 2 (IN, vừa trên min) → được chấp nhận                   | Not Run |
| TC-COUPON-ADMIN-021 | BVA            | min_order_amount = -1 (OFF, phải ≥ 0) → bị từ chối                       | Not Run |
| TC-COUPON-ADMIN-022 | BVA            | min_order_amount = 0 (ON, đúng min ≥ 0) → được chấp nhận                 | Not Run |
| TC-COUPON-ADMIN-023 | BVA            | min_order_amount = 1 (IN, vừa trên min) → được chấp nhận                 | Not Run |
| TC-COUPON-ADMIN-024 | BVA            | max_uses_per_user = 0 (OFF, phải ≥ 1) → bị từ chối                       | Not Run |
| TC-COUPON-ADMIN-025 | BVA            | max_uses_per_user = 1 (ON, đúng min ≥ 1) → được chấp nhận                | Not Run |
| TC-COUPON-ADMIN-026 | BVA            | max_uses_per_user = 2 (IN, vừa trên min) → được chấp nhận                | Not Run |
