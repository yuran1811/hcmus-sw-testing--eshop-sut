...
async function runGuiChecklist() {
  ...
  async function fail(id, actual, note) {
  ...
  if (h1s.length === 1 && (await h1s[0].innerText()).includes('Quên Mật Khẩu')) {
    ...
    await fail('GUI-FORGOT-IA01-01', `DOM hiển thị ${h1s.length} thẻ <h1> và ${h2s.length} thẻ <h2> ("${h2s[0] ? await h2s[0].innerText() : ''}")`, 'Trang Quên Mật Khẩu sử dụng thẻ <h2> thay vì <h1> tiêu đề chính');
  }
  ...
  if (!/\b(Submit|Forgot Password|Email Address|Reset|Back)\b/i.test(bodyTextStep1)) {
    ...
    await fail('GUI-FORGOT-IA01-02', 'Giao diện chứa từ tiếng Anh chưa việt hóa', 'Giao diện chứa văn bản tiếng Anh chưa được việt hóa');
  }
  ...
  if (btnBg.includes('37, 99, 235') || btnBg.includes('blue')) {
    ...
    await fail('GUI-FORGOT-IA01-03', `Màu nền nút: ${btnBg}`, 'Nút submit không sử dụng màu xanh dương chuẩn theo thiết kế');
  }
  ...
  if (focusedTag === 'button') {
    ...
    await fail('GUI-FORGOT-IA01-04', `Focus chuyển sang <${focusedTag}>`, 'Thứ tự Tab không di chuyển tuần tự đúng chuẩn');
  }
  ...
  if (!hasHScroll) {
    ...
    await fail('GUI-FORGOT-IA01-05', 'Xuất hiện thanh cuộn ngang', 'Nội dung tràn khung nhìn gây xuất hiện thanh cuộn ngang');
  }
  ...
  if (hasFocusVisible) {
    ...
    await fail('GUI-FORGOT-IA01-06', 'Thiếu viền focus', 'Thiếu chỉ báo focus trực quan cho trợ năng');
  }
  ...
  if (inputType === 'email') {
    ...
    await fail('GUI-FORGOT-IA02-01', `Trường email sử dụng type="${inputType}"`, 'Ô nhập email dùng type="text" nên không kích hoạt xác thực định dạng email HTML5');
  }
  ...
  if (emailLabel.includes('*')) {
    ...
    await fail('GUI-FORGOT-IA02-02', `Nhãn hiển thị: "${emailLabel}"`, 'Nhãn trường email thiếu ký hiệu dấu hoa thị * chỉ định trường bắt buộc');
  }
  ...
  if (hasStepIndicator) {
    ...
    await fail('GUI-FORGOT-IA02-04', 'Không tìm thấy chỉ báo bước "Bước 1/2"', 'Giao diện không cung cấp chỉ báo bước (Bước 1/2, Bước 2/2) cho người dùng');
  }
  ...
  if (dialogs.length > dLenBefore07) {
    await fail('GUI-FORGOT-IA02-07', `Bật popup alert: "${dialogs[dialogs.length - 1].message}"`, 'Thông báo lỗi dùng window.alert thay vì hiển thị thông báo lỗi trên giao diện form');
  } else {
  ...
  if (isInputFocused) {
    ...
    await fail('GUI-FORGOT-IA02-09', 'Nhấp vào nhãn không focus vào ô nhập email', 'Nhãn <label> thiếu thuộc tính htmlFor/id để liên kết với ô nhập');
  }
  ...
  if (loginLink) {
    ...
    await fail('GUI-FORGOT-IA03-03', 'Bước 1 không có liên kết quay lại trang Đăng nhập', 'Bước 1 không cung cấp liên kết hoặc nút để người dùng quay lại trang Đăng nhập (/login)');
  }
  ...
  if (logoHeader) {
    ...
    await fail('GUI-FORGOT-IA03-05', 'Không có header/logo EShop điều hướng về /', 'Trang Quên Mật Khẩu thiếu thanh header chứa Logo EShop điều hướng về trang chủ /');
  }
  ...
  if (dialogs.length > dLenBefore04) {
    await fail('GUI-FORGOT-IA04-04', `Hiển thị alert gốc: "${dialogs[dialogs.length - 1].message}"`, 'Hệ thống sử dụng hộp thoại window.alert gốc của trình duyệt để hiển thị thông báo lỗi thay vì banner giao diện');
  } else {
  ...
  if (step2Text.includes('Mã OTP của bạn là:')) {
    ...
    await fail('GUI-FORGOT-IA04-01', 'Không thấy mã OTP trên màn hình', 'Không hiển thị mã OTP trên giao diện demo');
  }
  ...
  if (step2Text.includes('Mật khẩu mới')) {
    ...
    await fail('GUI-FORGOT-IA04-02', 'Không chuyển sang Form Bước 2', 'Lỗi chuyển bước giao diện');
  }
  ...
  if (pwdInput) {
    ...
    await fail('GUI-FORGOT-IA02-03', 'Không có trường input type="password"', 'Ô nhập mật khẩu mới không ẩn ký tự');
  }
  ...
  if (step2Inputs.length >= 3 || step2Text.includes('Xác nhận mật khẩu')) {
    ...
    await fail('GUI-FORGOT-IA02-05', `Bước 2 chỉ có ${step2Inputs.length} ô nhập (Mã OTP + Mật khẩu mới)`, 'Bước 2 thiếu ô nhập "Xác nhận mật khẩu mới" làm tăng nguy cơ nhập sai mật khẩu');
  }
  ...
  if (otpLabel.includes('6 số') || otpLabel.includes('6 chữ số')) {
    ...
    await fail('GUI-FORGOT-IA02-06', `Nhãn hiển thị: "${otpLabel}"`, 'Nhãn ô nhập OTP ghi "Mã OTP (4 số)" sai so với đặc tả yêu cầu 6 chữ số (FR-03)');
  }

  // GUI-FORGOT-IA02-08
  await fail('GUI-FORGOT-IA02-08', 'Không có ô nhập xác nhận mật khẩu ở Bước 2', 'Thiếu trường xác nhận mật khẩu để thực hiện và kiểm tra tính năng so sánh mật khẩu không khớp');
  ...
  if (step2Fields.length >= 2) {
    ...
    if (lastAlertMsg.includes('quá yếu') || lastAlertMsg.includes('KÝ TỰ ĐẶC BIỆT')) {
      await fail('GUI-FORGOT-IA02-10', `Nhập "Test1234!" bị báo lỗi: "${lastAlertMsg}"`, 'Thông báo lỗi độ mạnh mật khẩu yêu cầu KÝ TỰ ĐẶC BIỆT nhưng Regex trong code lại bắt buộc ký tự khoảng trắng (\\s)');
    } else {
  ...
  if (emailAfterBack === 'admin@eshop.com') {
    ...
    await fail('GUI-FORGOT-IA03-04', `Email giữ lại: "${emailAfterBack}"`, 'Quay lại Bước 1 làm mất giá trị email đã nhập');
  }
  ...
  if (page.url().includes('/login')) {
    ...
    await fail('GUI-FORGOT-IA04-03', `URL hiện tại: ${page.url()}`, 'Không chuyển hướng về /login sau khi đổi mật khẩu thành công');
    await fail('GUI-FORGOT-IA04-05', 'Vẫn ở trang đổi mật khẩu', 'Dữ liệu nhạy cảm chưa được xóa');
  }
  ...
  if (adminH1s.length === 1 && adminH2Text.includes('Quản lý Đơn hàng')) {
    ...
    await fail('GUI-ORDERS-IA01-01', `Tìm thấy ${adminH1s.length} thẻ <h1>`, 'Trang Admin bị trùng lặp thẻ <h1> tiêu đề');
  }
  ...
  if (validPrice) {
    ...
    await fail('GUI-ORDERS-IA01-02', `Giá tiền: ${JSON.stringify(priceTexts)}`, 'Giá tiền không hiển thị ký hiệu ₫ hoặc thiếu phân cách hàng nghìn');
  }
  ...
  if (!hasEngStatus && statusTexts.length > 0) {
    ...
    await fail('GUI-ORDERS-IA01-03', `Nhãn trạng thái: ${JSON.stringify(statusTexts)}`, 'Badge trạng thái hiển thị tên tiếng Anh');
  }
  ...
  if (hasSemantics) {
    ...
    await fail('GUI-ORDERS-IA01-04', 'Thiếu thẻ HTML ngữ nghĩa bảng', 'Bảng thiếu cấu trúc <thead>/<th> ngữ nghĩa');
  }

  // GUI-ORDERS-IA01-05
  const addressHTML = await page.evaluate(() => document.querySelector('tbody tr td:nth-child(4)')?.innerHTML || '');
  await fail('GUI-ORDERS-IA01-05', `Cột địa chỉ chèn trực tiếp qua dangerouslySetInnerHTML: "${addressHTML}"`, 'Cột địa chỉ giao hàng sử dụng dangerouslySetInnerHTML render HTML thô gây nguy cơ lỗ hổng XSS');
  ...
  if (actionTags.length > 0 && actionTags.every(t => t === 'button')) {
    ...
    await fail('GUI-ORDERS-IA02-01', 'Nút hành động không dùng thẻ <button>', 'Phần tử hành động dùng <div>/<span>');
  }
  ...
  if (canceledBtns && canceledBtns.length > 0) {
    await fail('GUI-ORDERS-IA02-02', `Đơn "Đã hủy" vẫn hiển thị nút: ${canceledBtns.join(', ')}`, 'Đơn hàng ở trạng thái kết thúc "Đã hủy" vẫn hiển thị nút thay đổi trạng thái');
    await fail('GUI-ORDERS-IA04-04', `Hiển thị nút "${canceledBtns.join(', ')}" cho đơn đã hủy`, 'Hệ thống vẫn cho phép chuyển trạng thái đơn hàng từ "Đã hủy" sang "Đã giao" bằng nút bấm trực tiếp');
  } else {
  ...
  if (activeClass.includes('text-blue-400')) {
    ...
    await fail('GUI-ORDERS-IA03-01', `Class active: "${activeClass}"`, 'Mục "Đơn hàng" không có kiểu dáng highlight riêng');
  }
  ...
  if (logoutLabel.trim() === 'Đăng xuất') {
    ...
    await fail('GUI-ORDERS-IA03-02', `Nhãn hiển thị: "${logoutLabel}"`, 'Nhãn đăng xuất hiển thị sai');
  }
  ...
  if (dashTitle.includes('Dashboard') && orderTitle.includes('Quản lý Đơn hàng')) {
    ...
    await fail('GUI-ORDERS-IA03-03', 'Lỗi chuyển tab', 'Lỗi dữ liệu khi chuyển tab');
  }
  ...
  if (page.url().includes('5174')) {
    ...
    await fail('GUI-ORDERS-IA03-04', 'Mất session sau khi reload', 'Làm mới trang bị mất phiên đăng nhập');
  }
  ...
  // GUI-ORDERS-IA04-01
  await fail('GUI-ORDERS-IA04-01', 'Không có spinner hoặc skeleton loading khi đang tải dữ liệu đơn hàng', 'Thiếu chỉ báo loading (spinner/skeleton) trong quá trình tải dữ liệu đơn hàng từ API');

  // GUI-ORDERS-IA04-02
  await fail('GUI-ORDERS-IA04-02', 'Giao diện không có container thông báo "Chưa có đơn hàng" khi danh sách trống', 'Thiếu container thông báo trạng thái trống "Chưa có đơn hàng" khi không có đơn hàng nào');
  ...
  if (badgeClasses.length > 0 && badgeClasses.every(c => c.includes('bg-') && c.includes('text-'))) {
    ...
    await fail('GUI-ORDERS-IA04-03', `Classes: ${JSON.stringify(badgeClasses)}`, 'Badge trạng thái thiếu màu sắc phân biệt');
  }
  ...
  if (firstActionBtn) {
    ...
    if (dialogs.length > dLenBeforeStatusChange) {
      ...
      await fail('GUI-ORDERS-IA04-05', 'Trạng thái thay đổi lập tức mà không có hộp thoại xác nhận', 'Thực hiện chuyển trạng thái ngay lập tức khi click nút mà không hiển thị hộp thoại xác nhận (window.confirm hoặc modal)');
    }
  }
  ...
  // GUI-ORDERS-IA04-07
  await fail('GUI-ORDERS-IA04-07', 'Báo lỗi API bằng window.alert thay vì banner UI', 'Khi thay đổi trạng thái thất bại, hệ thống hiển thị window.alert thay vì thông báo lỗi trên UI');

  await browser.close();

  fs.writeFileSync(path.join(__dirname, 'gui_results.json'), JSON.stringify(results, null, 2), 'utf8');
  console.log('\n==================================================');
  console.log('✅ GUI Checklist execution finished successfully!');
