...
- [5. Phương án connection pool](#5-phương-án-connection-pool)
...
Horizontal scaling nhiều Node.js instance và connection pool thực sự phù hợp hơn khi chuyển sang PostgreSQL/MySQL. SQLite file và `userCarts` in-memory hiện tại tạo các giới hạn:

- writer contention;
- dữ liệu cart không dùng chung giữa process;
- khó scale ngang nhất quán;
- connection pool nhiều connection không tự giải quyết write serialization.

Không nên gắn nhãn “thêm connection pool” là Applicable trực tiếp cho stack hiện tại. Đây là phương án migration cần thiết kế database, session/cart storage, transaction và deployment trước khi triển khai.

## 5. Phương án connection pool

### 5.1. SQLite hiện tại

Khuyến nghị **không thêm pool PostgreSQL/MySQL một cách máy móc**. Module hiện tại tạo một `sqlite3.Database` singleton. Với workload homework/local, một connection được serialize có thể đơn giản và dễ kiểm soát hơn nhiều connection cạnh tranh cùng file.

Nếu cần kiểm tra nhiều connection, phải benchmark hai biến thể:

| Biến thể                            | Rủi ro                                                                                           |
| ----------------------------------- | ------------------------------------------------------------------------------------------------ |
| Một connection + WAL + busy timeout | Queue nội bộ có thể tăng khi write dồn, nhưng hành vi đơn giản.                                  |
| Nhiều connection                    | Có thể tăng read concurrency, nhưng tăng lock contention, checkpoint và khó quản lý transaction. |

Đo `p95`, `p99`, failure và lock wait trước khi chọn. Pool không phải thay thế cho index hoặc transaction đúng.

### 5.2. Khi chuyển sang PostgreSQL/MySQL
...
### Phase 4 - Kiến trúc mở rộng

Chỉ cân nhắc Redis, connection pool server database hoặc horizontal scaling sau khi workload mục tiêu vượt khả năng SQLite và đã có evidence từ profiling. Khi đó cần migration plan riêng, không coi đây là thay đổi nhỏ trong `database.js`.
...
Connection pool không phải giải pháp trực tiếp cho SQLite singleton hiện tại. Nếu cần pool và horizontal scaling, hướng đúng là thiết kế migration sang PostgreSQL/MySQL cùng transaction, session/cart storage và giới hạn pool phù hợp.
