...
| -------------------- | ------------------------------------------------------------------------------------------------------------ |
| **AI Tool**          | Antigravity IDE (Gemini 3.7 Flash)                                                                           |
| **Date/Time**        | 2026-08-14 23:18:00 +07:00                                                                                   |
...
```text
Thực hiện Bước 3: Sinh 3 file JMeter test plan (.jmx) cho Load, Stress, Spike:
- Quy ước đặt tên: 23127148_{ScenarioType}_20260815.jmx
- Cấu hình Ultimate Thread Group theo đúng workload-model.md
- Tích hợp CSV Data Set Config, HTTP Request Defaults (localhost:3000), HTTP Header Manager
- Dynamic ID Extractor cho POST /api/categories -> PUT /api/categories/:id
- Phân bổ tải 60% Read / 25% Transactional CRUD / 15% Bulk Import qua ThroughputController
- Think Time: Load (1-3s), Stress (1-3s), Spike (0s)
- 3 Listener khác nhau cho 3 file: Summary Report (Load), Aggregate Report (Stress), View Results Tree (Spike)
```
...
- **Các bước thực hiện:** AI đã thiết kế cấu trúc cây test plan JMeter bằng XML hợp lệ, cấu hình `kg.apc.jmeter.threads.UltimateThreadGroup`, `CSVDataSet`, `HeaderManager`, `JSONPostProcessor` trích xuất `token` và `createdCategoryId`, gán `ThroughputController` (60/25/15%), áp dụng `UniformRandomTimer` và bổ sung 3 Listener riêng biệt cho từng kịch bản.
...
1. [`HW5/test-plans/23127148_Load_20260815.jmx`](file:///d:/Project/Testing/hcmus-sw-testing--eshop-sut/HW5/test-plans/23127148_Load_20260815.jmx):
   ...
   - Uniform Random Timer: 1–3 giây.
   - Listener: **Summary Report** (`SummaryReportGui`).
...
| **Student Fix**    | Sinh viên đã rà soát lại cấu trúc XML, xác nhận các biến `${token}` và `${createdCategoryId}` được scope đúng trong Header Manager cục bộ của từng nhánh giao dịch, kiểm tra đường dẫn CSV tương đối `../test-data/*.csv` và xác thực 3 Listener khác nhau.                               |
| **Reviewed by**    | Ân Tiến Nguyên An                                                                                                                                                                                                                                                                          |
| **Review date**    | 2026-08-14                                                                                                                                                                                                                                                                                 |
