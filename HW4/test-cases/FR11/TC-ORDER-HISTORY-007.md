# TC-ORDER-HISTORY-007: Verify shipping status displays 'Đang giao' with blue styling
...
| Purpose | Verify shipping status displays 'Đang giao' with blue styling |
...
| Amount | 300000 |
| Expected label | Đang giao |
| Expected class | bg-blue-100 text-blue-800 |
...
3. Use/verify an order fixture with amount `300000`.
4. Inspect order rows, status labels, action buttons, navigation state, or empty-state content as required by the case.

## Expected Result

Expected label/message: `Đang giao`. Expected CSS class/styling: `bg-blue-100 text-blue-800`.

## Automation Mapping

- Case ID in data file: `F11-TC-007`
