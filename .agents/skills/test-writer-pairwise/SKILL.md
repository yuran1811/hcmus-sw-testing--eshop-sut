...
3. **Bắt buộc Sinh REST Client Scripts (.rest)**:
   ...
   - Đường dẫn file: `tests/test-runs/script/{MODULE}/TC-{MODULE}-PWS-{ID}.rest`
...
| **Output Summary**         | Generated {N} Pairwise TCs, Test Run table, and REST scripts |
...
1. Thông báo đã cập nhật `Pair_Wise_Testing.md`, file Test Run (`{MODULE}-test-run.md`), các file REST script (`tests/test-runs/script/{MODULE}/*.rest`), và `AI_Audit_Report.md`.
...
- [DO] Kiểm tra xem AI đã xác định các giá trị đại diện (Values) cho từng Parameter hợp lý chưa.
